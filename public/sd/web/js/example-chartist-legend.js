function myFunction() {
  ...
  function add()
      ...
      if(tags.length<16){
          if(document.getElementById("inp").value != "" && tags.indexOf(document.getElementById("inp").value)<0){
              tags.push({tag: document.getElementById("inp").value, avg:0, min:null, max:null,total:0,count:0});
  ...
  function deleteR(row){
      ...
      if (xmlhttp.readyState!=4){
          setTimeout(function(){ trdData.series.splice(row-1,1) }, 2000)
      } else { 
      ...
      if (toggleV == 0){
          ...
          toggleV = 1
              ...
              for (series of trdData.series){
                  ...
                  series.data.push({x: new Date(ts.getTime()), y: null})
  ...
  function buildRest(item) {
      ...
      if(index==-1) {
      trdData.series.push({name: item.tag,
        data: []})}
  ...
  xmlhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
        ...
        var tbl = document.getElementById("tLegend");
          ...
          for (x of keys) {
            trdData.series[idx].data.push({x: new Date(ts.getTime()), y: values[x]})
  ...
  var data = 
      {
        name: 'series-1',
        data: [
          {x: new Date(msSinceEpoch -60000), y: null},
          {x: new Date(msSinceEpoch), y: null}
        ]
      };
